...
class CustomerAccount:
    ...
    def total_due(self):
        return sum(map(lambda invoice: invoice.amount_due(), self.invoices))

    def unpaid_invoices(self):
        return list(filter(lambda invoice: invoice.amount_due() > 0, self.invoices))

    def apply_payment(self, payment):
        while payment != 0:
            invoice = self.unpaid_invoices()[0]
            invoice_payment = invoice.amount_due() if payment >= invoice.amount_due() else payment
