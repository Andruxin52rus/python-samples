...
class ChickenInfoView(JSONView):
    def data(self) -> dict:
        return {'calories': 231, 'fat': 5, 'protein': 43, 'serving_size': 140}

class HTMLView(View):
    body = ''
    def render(self) -> str:
        return '<html><body>{}</body></html>'.format(self.body)
...
class WebApp:
    def __init__(self):
        self._routes = dict()
    def add_route(self, URL: str, page_view: View):
        self._routes[URL] = page_view
    def get(self, URL: str) -> str:
        return self._routes[URL].render()
    def urls(self):
