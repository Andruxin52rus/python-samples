...
def most_spaces(items):
    return max(items, key=lambda item: item.count(" "))


def fewest_vowels(items):
    return min(items, key=lambda item: sum([item.lower().count(vowel) for vowel in VOWELS]))


def most_consonants(items):
    return max(items, key=lambda item: sum([item.lower().count(consonant) for consonant in CONSONANTS]))


def sorted_by_word_count(items):
    return sorted(items, key=lambda item: len(item.split(" ")))
